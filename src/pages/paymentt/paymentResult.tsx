    ...
    if (paymentMethods == "vnpay") {
      ctqmService.orderApi
        ...
        .then((rs) => {
          ...
          if (rs.message == "redirect") {
            // window.open(rs.url);
            window.open(rs.url, "Payment Popup", "width=600,height=400");
            ctqmService.orderApi
            .vnPayPayment("cbbf1eb6-ddb4-4577-82c4-56dd9e9fc7fd")
            .then((rs) => {
              console.log("SDKJFNWJENRIWERiuWEUIR ", rs);
              if (rs.message == "PaymentSuccess") {
                console.log("SDKJFNWJENRIWERiuWEUIRasdfjiqwehriuwer");
              }
            });
          }
        })
    ...
    <React.Fragment>
      ...
      <main>
        <div className="w-full bg-white border-t border-b border-gray-200 px-5 py-10 text-gray-800">
          ...
          {iframeContent != "" ? (
            <iframe
              src={iframeContent} // Sử dụng srcDoc thay vì src để render nội dung của iframe
              width="100%"
              height="auto"
            />
          ) : (
